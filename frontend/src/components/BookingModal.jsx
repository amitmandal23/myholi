...
const BookingModal = ({ isOpen, onClose, packageName, title = "Want to Go For A Memorable Holiday?" }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const inquiryData = {
      ...
      message: `Inquiry for ${packageName}. Adults: ${formData.adults}, Children: ${formData.children}, Infants: ${formData.infants}`,
      status: 'pending'
    };
        ...
        <form onSubmit={handleSubmit} className="p-6 pt-2 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {['Adult', 'Child', 'Infant'].map((type) => {
              const field = type === 'Adult' ? 'adults' : type === 'Child' ? 'children' : 'infants';
              return (
