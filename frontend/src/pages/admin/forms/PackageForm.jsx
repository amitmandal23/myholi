...
import GalleryImageGrid from "../../../components/common/GalleryImageGrid";
...
const PackageForm = () => {
    ...
    try {
      ...
      const toHtml = (val) => {
        ...
        if (Array.isArray(val))
          return `<ul>${val.map((i) => `<li>${i}</li>`).join("")}</ul>`;
        try {
          const parsed = JSON.parse(val);
          if (Array.isArray(parsed))
            return `<ul>${parsed.map((i) => `<li>${i}</li>`).join("")}</ul>`;
          return val;
        } catch {
          return val;
        }
      ...
      setFormData({
        ...
        hotel_details:
          typeof data.hotel_details === "object" && data.hotel_details !== null
            ? JSON.stringify(data.hotel_details, null, 2)
            : data.hotel_details || "",
        itinerary:
          typeof data.itinerary === "object" && data.itinerary !== null
        ...
        show_in_menu:
          data.show_in_menu !== undefined ? Boolean(data.show_in_menu) : true,
      });
      setFetching(false);
    } catch (err) {
  ...
  const handleDeleteGalleryImage = async (imgUrl) => {
    if (!id || !window.confirm("Remove this image?")) return;
    setDeletingImage(imgUrl);
    try {
      const body = new FormData();
      body.append("image_url", imgUrl);
      const response = await fetch(API_ENDPOINTS.PACKAGE_GALLERY_REMOVE(id), {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
      ...
      setFormData((prev) => ({
        ...prev,
        images: JSON.stringify(result.images, null, 2),
      }));
    } catch (err) {
  ...
  useEffect(() => {
    if (formData.itinerary) {
      try {
        const parsed =
          typeof formData.itinerary === "string"
                  ...
                  <img
                    src={
                      formData.featured_image.startsWith("http")
                        ? formData.featured_image
                        : `${IMAGE_BASE_URL}${formData.featured_image}`
                    }
          ...
          <div>
            ...
            <div className="space-y-4 border rounded-lg p-4 bg-gray-50 max-h-[600px] overflow-y-auto">
              {itineraryItems.map((item, index) => (
                <div
                  key={index}
                  className="bg-white p-4 rounded border shadow-sm relative"
                >
                    ...
                    <RichTextEditor
                      value={
                        ...
                        (Array.isArray(item.activities)
                          ? `<ul>${item.activities.map((a) => `<li>${a}</li>`).join("")}</ul>`
                          : item.description || "")
                      }
                      onChange={(value) =>
                        updateItineraryDay(index, "content", value)
                      }
